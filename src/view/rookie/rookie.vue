...
import { getRookies, addRookie, editRookie, deleteRookie } from '@/api/notice'
export default {
  name: 'product',
  components: {
    Tables,
                ...
                on: {
                  click () {
                    const config = {
                      ...
                      loading: true,
                      onOk: () => {
                        deleteRookie({ id: params.row.id }).then(res => {
                          _self.$Modal.remove()
  ...
  methods: {
    ...
    async getTableData (pager, querys) {
      const res = await getRookies(pager, querys)
    ...
    async onFormsSubmit (event) {
      ...
      if (this.formsTitle.includes('编辑')) {
        data.id = event.id
        const res = await editRookie(data)
        this.$Message.info(res.data.message)
        this.getTableData(this.pageParams, this.queryParams)
        this.formsVisible = false
      }
      if (this.formsTitle.includes('添加')) {
        const res = await addRookie(data)
        this.$Message.info(res.data.message)
        this.getTableData(this.pageParams, this.queryParams)
        this.formsVisible = false
      }
